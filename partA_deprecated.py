...
def check_unsupported_chars(alphabet, plaintext):
    # convert the uppercase to lowercase for encryption
    plaintext_lower = plaintext.lower()
    # detect and remove unsupported chars:
    unsupp_chars = set(plaintext_lower).difference(alphabet)
    for unsupp_char in unsupp_chars:
        plaintext_lower = plaintext_lower.replace(unsupp_char, '')
    return plaintext_lower
...
# plaintext = check_unsupported_chars(alphabet, plaintext)
plaintext = 'texten måste ju vara på svenska, sa någon. vem det var kan jag inte svara på. nu ska jag visa er att ' \
            'jag kan måla upp en historia som blir lika innehållsrik som en historia som innehåller frågor, fast ' \
            'min historia kommer inte innehålla frågor... ni kommer förstå varför och hur jag gjorde det. men en ' \
            'magiker kan inte avslöja sina trick innan den uppträder, så ni får ta en sekund och luta er tillbaka, ' \
            'njut av showen. det var en gång en liten pojke som hette einar. nu vet jag att ni undrar vem einar är ' \
            'och varför jag tog upp honom. jo då ska ni veta en sak, och det är att ni inte kommer få veta det. ' \
            'tack för mig.'
plaintext = check_unsupported_chars(alphabet, plaintext)
